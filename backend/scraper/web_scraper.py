...
class WebScraper:
    ...
    async def scrape_file(self, file_path: str) -> None:
        """
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                for i, section in enumerate(sections):
                    ...
                    id = f"{file_path}-{i}"  # Use file path and section number for unique ID
                    metadata = {
                        'title': section['title'],
                        'summary': section['summary'],
                        'content': section['content'],
                ...
                self.index.upsert(vectors=pinecone_data, namespace=NAMESPACE)
                print(f"Scraped and stored embeddings for {file_path}")
        except Exception as e:
    ...
    async def scrape_files(self, file_paths: List[str]) -> None:
        """Scrape a list of local HTML files and store the scraped data in Pinecone."""
